...
class Player:
    ...
    def autoVelocityChange(self, hamiltonCycleGrid, applePosition):        
        ...
        for velocity in Player._velocities:
            ...
            currentLocation = hamiltonCycleGrid[self.position[0]][self.position[1]]
            ...
            # check for movement against cycle
            againstCycle = False
            if nextLocation < currentLocation and currentLocation != NUMBEROFCELLS-1 and nextLocation != 0:
               againstCycle = True
            ...
            if againstCycle:
                possiblePositions.append((2, distance, velocity))
            else:
                possiblePositions.append((2, distance, velocity))
        
        # pick path with smallest distance from apple
        try:
            self.velocity = Player._velocities[min(possiblePositions)[2]]
        except:
