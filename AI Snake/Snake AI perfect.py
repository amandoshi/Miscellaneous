...
class Player:
    ...
    def autoVelocityChange(self, hamiltonCycleGrid, applePosition):        
        # next location in hamilton cycle
        targetLocation = (hamiltonCycleGrid[self.position[0]][self.position[1]] + 1) % NUMBEROFCELLS
        ...
        for velocity in Player._velocities:
            ...
            # if next location matches target, set player velocity
            nextLocation = hamiltonCycleGrid[nextPosition[0]][nextPosition[1]]
            if nextLocation == targetLocation:
                self.velocity = Player._velocities[velocity]
                break
